...
import {
  User, Mail, Gift, Copy, Check, Sparkles, Ticket, LogOut, ChevronRight,
  CalendarDays, MapPin, Clock, CheckCircle2, AlertTriangle, XCircle,
  Pencil, Lock, Save, X,
} from "lucide-react";
import {
  fetchProfile, fetchMyTransactions, updateProfile, changePassword,
  type ApiTransactionListItem, type TransactionStatus,
} from "@/utils/api";
...
function formatDate(s: string, withTime = false): string {
  const d = new Date(s);
  if (withTime) {
...
const statusConfig: Record<TransactionStatus, { label: string; color: string; bg: string; icon: typeof Clock }> = {
  WAITING_FOR_PAYMENT: { label: "Menunggu Pembayaran", color: "text-yellow-400", bg: "bg-yellow-500/10", icon: Clock },
  WAITING_FOR_CONFIRMATION: { label: "Menunggu Konfirmasi", color: "text-blue-400", bg: "bg-blue-500/10", icon: Clock },
  DONE: { label: "Berhasil", color: "text-emerald-400", bg: "bg-emerald-500/10", icon: CheckCircle2 },
  REJECTED: { label: "Ditolak", color: "text-red-400", bg: "bg-red-500/10", icon: XCircle },
  EXPIRED: { label: "Kedaluwarsa", color: "text-white/40", bg: "bg-white/5", icon: AlertTriangle },
  CANCELED: { label: "Dibatalkan", color: "text-white/40", bg: "bg-white/5", icon: XCircle },
};
...
export default function ProfilePage() {
  ...
  const { data: trxData } = useQuery({
    queryKey: queryKeys.transactions.my({ limit: 20 }),
    queryFn: () => fetchMyTransactions({ limit: 20 }),
  });
  ...
  const updateProfileMutation = useMutation({
    mutationFn: (vars: { name?: string; file?: File }) => updateProfile({ name: vars.name }, vars.file),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.profile });
      setProfileFile(null);
      setProfilePreview(null);
      setEditSuccess("Profil berhasil diupdate!");
      setTimeout(() => { setEditSuccess(""); setEditOpen(false); }, 1500);
    },
  ...
  const changePasswordMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setTimeout(() => { setPwSuccess(""); setPwOpen(false); }, 1500);
    },
    onError: async (err: any) => {
      try {
        const body = await err.response?.json();
  ...
  const handleEditSubmit = () => {
    ...
    setEditError("");
    setEditSuccess("");
    updateProfileMutation.mutate({ name: editForm.name, file: profileFile || undefined });
  };

  const handlePwSubmit = () => {
    if (!pwForm.currentPassword) { setPwError("Password lama wajib diisi"); return; }
  ...
  return (
    <BrowseLayout>
      ...
      <main className="page-wrap py-6 sm:py-10">
        ...
        <div className="rise-in flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <h1 className="display-title text-2xl font-bold text-white sm:text-3xl">{profile.name}</h1>
        ...
        {editOpen && (
          <section className="rise-in mt-6 rounded-2xl border border-white/12 bg-white/5 p-5 sm:p-6">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setEditOpen(false)} className="text-xs font-semibold text-white/50 hover:text-white">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="mt-4 space-y-4">
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/50">Nama</label>
              ...
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/50">Foto Profil</label>
                <label className="flex cursor-pointer items-center gap-4 rounded-xl border border-white/12 bg-white/5 px-3.5 py-3 transition-all hover:border-white/30">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-full bg-white/10">
                    {profilePreview || profile.profileImage ? (
                      <img src={profilePreview || profile.profileImage || ""} alt="Preview" className="h-full w-full object-cover" />
                    ) : (
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/30">JPG, PNG, WebP — max 2MB</p>
                  </div>
                  <input
                    type="file"
              ...
              {editSuccess && <p className="text-xs font-semibold text-emerald-400">{editSuccess}</p>}
            ...
            <div className="mt-5 border-t border-white/10 pt-5">
              ...
              {pwOpen && (
                <div className="mt-4 space-y-4">
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/50">Password Lama</label>
                    <input
                      type="password"
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/50">Password Baru</label>
                    <input
                      type="password"
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/50">Konfirmasi Password Baru</label>
                    <input
                      type="password"
                  ...
                  {pwSuccess && <p className="text-xs font-semibold text-emerald-400">{pwSuccess}</p>}
        ...
        <div className="rise-in mt-6 grid grid-cols-1 gap-3 sm:mt-8 sm:grid-cols-3 sm:gap-4" style={{ animationDelay: "60ms" }}>
          <div className="rounded-2xl border border-white/8 bg-gradient-to-br from-[#2f6a4a]/20 to-transparent p-5">
            ...
            <p className="mt-1 text-xs text-white/40">Expire dalam 3 bulan setelah diperoleh</p>
          </div>
          ...
          <div className="rounded-2xl border border-white/8 bg-gradient-to-br from-[#328f97]/20 to-transparent p-5">
            ...
            <p className="mt-2 text-2xl font-bold text-white sm:text-3xl">{profile.coupons.length}</p>
          ...
          <div className="rounded-2xl border border-white/8 bg-gradient-to-br from-[#6366f1]/20 to-transparent p-5">
            ...
            <p className="mt-2 text-2xl font-bold text-white sm:text-3xl">{transactions.length}</p>
            <p className="mt-1 text-xs text-white/40">Total semua waktu</p>
          </div>
        </div>

        <section className="rise-in mt-6 rounded-2xl border border-white/8 bg-white/4 p-5 sm:mt-8 sm:p-6" style={{ animationDelay: "120ms" }}>
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-white/60">
            <Gift size={14} />
            Referral Code
          </div>
          <p className="mt-2 text-sm text-white/50">
            Bagikan kode ini ke teman. Kamu dapat 10.000 points, teman kamu dapat kupon diskon Rp 50.000.
          </p>

          <div className="mt-4 flex items-stretch gap-2">
            <div className="flex flex-1 items-center rounded-xl border border-white/12 bg-white/5 px-4 py-3">
              <code className="font-mono text-lg font-bold tracking-widest text-white">{profile.referralCode}</code>
            </div>
            <button
              onClick={handleCopyReferral}
              className={`flex shrink-0 items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-bold transition-all ${
                copied ? "bg-emerald-500/20 text-emerald-400" : "bg-white text-[#0a0a0a] hover:bg-white/90"
              }`}
            >
        ...
        {profile.coupons.length > 0 && (
          <section className="rise-in mt-6 sm:mt-8" style={{ animationDelay: "180ms" }}>
            <h2 className="text-sm font-bold uppercase tracking-wider text-white/70">Kupon Aktif</h2>
            <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
              {profile.coupons.map((coupon) => (
                <div key={coupon.id} className="relative overflow-hidden rounded-2xl border border-dashed border-white/20 bg-gradient-to-br from-[#d4537e]/15 to-transparent p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wider text-pink-400">Diskon</p>
                      <p className="mt-1 text-xl font-bold text-white">{formatPrice(coupon.discountAmount)}</p>
                  ...
                  <p className="mt-3 text-xs text-white/40">Berlaku sampai {formatDate(coupon.expiresAt)}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        <section className="rise-in mt-8 sm:mt-10" style={{ animationDelay: "240ms" }}>
          <h2 className="text-lg font-bold text-white sm:text-xl">Riwayat Transaksi</h2>
      ...
      <footer className="mt-10 border-t border-white/8 sm:mt-12">
        <div className="page-wrap flex flex-col items-center gap-4 py-6 text-sm text-white/30 sm:flex-row sm:justify-between sm:py-8">
          ...
          <div className="flex gap-6">
            ...
            <a href="#" className="text-white/30 no-underline hover:text-white/50">Help</a>
          </div>
        </div>
      </footer>
    </BrowseLayout>
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          <p className="truncate text-sm font-bold text-white sm:text-base">{trx.event.name}</p>
          <p className="shrink-0 text-sm font-bold text-white sm:text-base">{formatPrice(trx.finalPrice)}</p>
        </div>
        <p className={`mt-0.5 text-xs font-semibold ${config.color}`}>{config.label}</p>
        ...
        <p className="mt-1 text-xs text-white/30">{formatDate(trx.createdAt, true)}</p>
      </div>

      <ChevronRight size={18} className="shrink-0 text-white/20 transition-transform group-hover:translate-x-0.5" />
    </Link>
  );
}
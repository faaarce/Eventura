...
import {
  ArrowLeft, Users, Mail, Ticket, Download, Search,
} from "lucide-react";
import { fetchEventAttendees, type ApiAttendee } from "@/utils/api";
...
export default function DashboardAttendees() {
  const { eventId } = useParams<{ eventId: string }>();
  const [search, setSearch] = useState("");

  const { data, isLoading, error } = useQuery({
    queryKey: queryKeys.events.attendees(eventId!),
    queryFn: () => fetchEventAttendees(eventId!, { limit: 100 }),
    enabled: !!eventId,
  });
  ...
  const exportCSV = () => {
    const headers = ["Name", "Email", "Invoice", "Tickets", "Quantity", "Total Paid", "Purchased At"];
    ...
    link.href = url;
    link.setAttribute("download", `attendees-${eventName.replace(/\s+/g, "-").toLowerCase()}.csv`);
    link.click();
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-white/40">Attendees untuk</p>
          <h1 className="display-title mt-1 text-2xl font-bold text-white sm:text-3xl">{eventName}</h1>
          <p className="mt-1 text-sm text-white/50">Daftar customer yang udah beli tiket</p>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard label="Total Attendees" value={total.toLocaleString("id-ID")} color="indigo" />
        <StatCard label="Total Tiket" value={totalTickets.toLocaleString("id-ID")} color="teal" />
        <StatCard label="Total Revenue" value={formatPrice(totalRevenue)} color="emerald" />
      </div>
      ...
      {filtered.length > 0 ? (
        ...
        <div className="mt-6 rounded-2xl border border-white/8 bg-white/4 py-10 text-center">
          <p className="text-sm font-semibold text-white/60">Tidak ada hasil untuk "{search}"</p>
        </div>
      ) : (
        <div className="mt-6 rounded-2xl border border-white/8 bg-white/4 py-14 text-center">
          ...
          <p className="mt-4 text-base font-semibold text-white">Belum ada attendees</p>
...
function StatCard({
  label, value, color,
}: { label: string; value: string; color: "indigo" | "teal" | "emerald" }) {
  const gradients = {
  ...
  return (
    <div className={`rounded-2xl border border-white/8 bg-gradient-to-br ${gradients[color]} to-transparent p-5`}>
      <p className={`text-xs font-semibold uppercase tracking-wider ${textColors[color]}`}>{label}</p>
      <p className="mt-2 text-2xl font-bold text-white">{value}</p>
    </div>
  );
}

function AttendeeRow({ attendee }: { attendee: ApiAttendee }) {
  return (
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="text-sm font-bold text-white sm:text-base">{attendee.name}</p>
            <p className="mt-0.5 flex items-center gap-1.5 text-xs text-white/50">
              <Mail size={11} />
              <span className="truncate">{attendee.email}</span>
            </p>
            <p className="mt-0.5 font-mono text-xs text-white/30">{attendee.invoiceNumber}</p>

            <div className="mt-2 flex flex-wrap gap-1.5">
              {attendee.tickets.map((ticket, i) => (
                <span key={i} className="inline-flex items-center gap-1 rounded-full bg-white/8 px-2.5 py-0.5 text-xs text-white/70">
                  <Ticket size={10} />
        ...
        <div className="flex flex-col items-start gap-1 sm:items-end">
          ...
          <p className="text-lg font-bold text-white">{formatPrice(attendee.totalPaid)}</p>
          <p className="text-xs text-white/30">{formatDate(attendee.purchasedAt)}</p>
        </div>
      </div>
    </div>
  );

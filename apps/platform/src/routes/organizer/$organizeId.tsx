...
export const Route = createFileRoute("/organizer/$organizeId")({
  ...
  loader: async ({ params }) => {
    const [organizer, eventsData] = await Promise.all([
      fetchOrganizerProfile(params.organizeId),
      fetchEvents({ organizerId: params.organizeId, limit: 50 }),
    ]);
    return { organizer, events: eventsData.events };
  },
        ...
        {events.length === 0 && (
          <div className="mt-6 rounded-2xl border border-white/8 bg-white/4 py-14 text-center">
            ...
            <p className="mt-1 text-sm text-white/40">
              Organizer ini belum membuat event
            </p>
          </div>
        )}

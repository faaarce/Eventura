...
function NewEventPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (new Date(form.startDate) <= new Date()) {
      setError("Tanggal mulai harus di masa depan");
      return;
    }

    // Validasi ticket types
    for (let i = 0; i < ticketTypes.length; i++) {
      const t = ticketTypes[i];
      if (!t.name) {
        setError(`Nama ticket type #${i + 1} wajib diisi`);
        return;
      }
      if (!t.totalSeats || parseInt(t.totalSeats) < 1) {
        setError(`Jumlah kursi ticket type #${i + 1} minimal 1`);
        return;
      }
      if (!form.isFree) {
        if (t.price === "" || parseInt(t.price) < 0) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <p className="mt-1 text-sm text-white/50">
        Isi detail event kamu di bawah ini
      </p>
          ...
          <label className="mt-4 flex cursor-pointer items-center gap-3 rounded-xl border border-white/12 bg-white/5 p-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/40">
                Semua tiket akan diset ke harga Rp 0
              </p>
            </div>
          </label>
            ...
            {ticketTypes.map((ticket, index) => (
              ...
              >
                ...
                <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
                  ...
                  <FormField
                    label={`Harga (Rp)${form.isFree ? " — Gratis" : ""}`}
                    compact
                  >
                    <input
                      type="number"
                  ...
                  <FormField label="Jumlah Kursi" compact>
                    <input
                      ...
                      onChange={(e) =>
                        updateTicket(index, "totalSeats", e.target.value)
                      }
                      min={1}
                      className="form-input-sm"
                    />
...
function FormField({
  ...
  return (
    <div>
      <label
        className={`mb-1.5 block font-semibold uppercase tracking-wider text-white/50 ${
